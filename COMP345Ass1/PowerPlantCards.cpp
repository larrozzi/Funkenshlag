...
PowerPlantCards::PowerPlantCards(int cardValue, int numbResource, int capacity, int powerHouse) : cardValue(cardValue), capacity(capacity), powerHouse(powerHouse) {/*NOTHING*/}
PowerPlantCards::~PowerPlantCards() {}
...
inline int PowerPlantCards::getCapacity() const { return capacity; }
...
{
    outs << "Card Value: " << card.getCardValue() << "Number of Resource: " << card.getNumbResource()
    << ", Capacity: " << card.getCapacity() << ", PowerHouse: " << card.getPowerHouse();
...
std::vector<PowerPlantCards>createCards()
{
    std::vector<PowerPlantCards>myCards (43);
...
void PowerPlantCards::printCards()
{
    for (PowerPlantCards out : createCards()) {
